<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('orders', function (Blueprint $table) {
            $table->id();
            $table->integer('user_id')->default('0');
            $table->integer('branch_id')->default('0');
            $table->integer('driver_id')->default('0');
            $table->string('code_order')->nullable();
            $table->string('dri_time')->nullable();
            $table->integer('order_status')->default('0');
            $table->string('dri_name')->nullable();
            $table->string('dri_type')->nullable();
            $table->string('dri_no_car')->nullable();
            $table->string('dri_phone')->nullable();
            $table->string('dri_img')->nullable();
            $table->text('order_address')->nullable();
            $table->double('amount', 8, 2)->default('0.00');
            $table->double('price', 8, 2)->default('0.00');
            $table->string('b_name')->nullable();
            $table->string('b_recive_name')->nullable();
            $table->string('b_phone')->nullable();
            $table->string('b_address')->nullable();
            $table->string('o_name')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('orders');
    }
};
